...
function lookup_client_name() {
    ...
    echo "🔍 Looking up client for MSISDN: $msisdn in tenant: $tenant_id..." >&2
    ...
    # Parse JSON to extract display name (using basic text processing)
    # This looks for "displayName" field in the JSON response
    local display_name
    display_name=$(echo "$response" | grep -o '"displayName":"[^"]*"' | sed 's/"displayName":"\([^"]*\)"/\1/' | head -n1)
...
payer_name=$(lookup_client_name "$payer_msisdn" "$tenant_id")
payee_name=$(lookup_client_name "$payee_msisdn" "$tenant_id")

# Display payment details
echo -e "${BLUE}=== Payment Details ===${RESET}"
echo -e "${YELLOW}Payer:${RESET} $payer_name ($payer_msisdn)"
echo -e "${YELLOW}Payee:${RESET} $payee_name ($payee_msisdn)"
